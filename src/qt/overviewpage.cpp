...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLEEAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLEEAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLEEAmount = strAnonymizeLEEAmount.remove(strAnonymizeLEEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLEEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLEEAmount*COIN) nMaxToAnonymize = nAnonymizeLEEAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLEEAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLEEAmount));
        strAnonymizeLEEAmount = strAnonymizeLEEAmount.remove(strAnonymizeLEEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLEEAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
